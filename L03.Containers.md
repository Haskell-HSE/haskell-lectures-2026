...
`IntSet` is a specialised version of `Set` with more efficient allocation
pattern and more efficient tree layout &mdash; more precisely, they're not
balanced binary trees, but rather big-endian patricia trees where each parent
has much more than just two children, allowing to have near-O(1) time complexity
for its operations.
